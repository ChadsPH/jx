...
{
	public static final String EXTRA_HIDELOG = "com.chadx.injector.showNoLogWindow";
	...
	private void askForPW(final int type) {
        ...
        ((EditText) userpwlayout.findViewById(R.id.username)).setText(mConfig.getPrivString(Setting.USUARIO_KEY));
        ((EditText) userpwlayout.findViewById(R.id.password)).setText(mConfig.getPrivString(Setting.SENHA_KEY));
        ...
        ((ImageButton) userpwlayout.findViewById(R.id.show_password)).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (isMostrarSenha) {
					...
					((ImageButton) userpwlayout.findViewById(R.id.show_password)).setImageDrawable(ContextCompat.getDrawable(LaunchVpn.this, R.drawable.ic_cloud_outline));
				}
				else {
			...
			new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					if (type == R.string.password) {
						...
						edit.putString(Setting.USUARIO_KEY, mUsername);
						...
						if (((CheckBox) userpwlayout.findViewById(R.id.save_password)).isChecked()) {
							edit.putString(Setting.SENHA_KEY, pw);
						} else {
							edit.remove(Setting.SENHA_KEY);
							mTransientAuthPW = pw;
						}
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == START_VPN_PROFILE) {
            if (resultCode == Activity.RESULT_OK) {
				...
				if (!TunnelUtils.isNetworkOnline(this)) {
				...
				else if (prefs.getInt(Setting.TUNNELTYPE_KEY, Setting.bTUNNEL_TYPE_SSH_DIRECT) == Setting.bTUNNEL_TYPE_SSH_PROXY &&
						(mConfig.getPrivString(Setting.PROXY_IP_KEY).isEmpty() || mConfig.getPrivString(Setting.PROXY_PORTA_KEY).isEmpty())) {
					SkStatus.updateStateString("USER_VPN_PASSWORD_CANCELLED", "", R.string.state_user_vpn_password_cancelled,
						ConnectionStatus.LEVEL_NOTCONNECTED);
				...
				else if (!prefs.getBoolean(Setting.PROXY_USAR_DEFAULT_PAYLOAD, true) && mConfig.getPrivString(Setting.CUSTOM_PAYLOAD_KEY).isEmpty()) {
					SkStatus.updateStateString("USER_VPN_PASSWORD_CANCELLED", "", R.string.state_user_vpn_password_cancelled,
						ConnectionStatus.LEVEL_NOTCONNECTED);
				...
				else if (mConfig.getPrivString(Setting.SERVIDOR_KEY).isEmpty() || mConfig.getPrivString(Setting.SERVIDOR_PORTA_KEY).isEmpty()) {
					SkStatus.updateStateString("USER_VPN_PASSWORD_CANCELLED", "", R.string.state_user_vpn_password_cancelled,
						ConnectionStatus.LEVEL_NOTCONNECTED);
        			...
        			startLW.setComponent(new ComponentName(this, "com.chadx.injector.activities.ConfigGeralActivity"));
            	...
            	else if (mConfig.getPrivString(Setting.USUARIO_KEY).isEmpty() || (mConfig.getPrivString(Setting.SENHA_KEY).isEmpty() &&
